...
from models.session_model import db, User
from werkzeug.security import generate_password_hash, check_password_hash

bp_session = Blueprint(__name__, "session")
...
def register():
    ...
    auth.register_new_user(user_info)
...
def login():
    data = request.get_json()
    required_fields = [identifier, password] # identifier can be either email or username
    login_info = {field: None for field in required_fields}
    
    existing_user = User.query.filter_by(username=login_info[identifier]).first()
    if existing_user is None:
        ...
        if check_password_hash(existing_user.password_hash, login_info[password]):
            ...
            return jsonify("message": f"User {login_info[identifier]} logged in successfully"), 200
        else:
...
@bp_session.route('/profile/<str:user_id>', methods=['GET'])
@login_required()
def get_profile(user_id):
    ...
    user_profile = {
        ...
        "Password": authenticated_user.password_hash # user should be able to see their plain password
    }
    return jsonify({"message": f"{Username}'s profile retrieved successfully",
                    "data": user_profile}), 200
